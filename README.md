...
The stack includes **MySQL 8**, **app** (nginx + PHP-FPM), **queue**, and **scheduler**. Named volumes: **`mysql`** (database data), **`storage`** (uploads, logs, generated `APP_KEY` file), **`bootstrap_cache`**. Migrations live inside the image—do not mount a volume over `/var/www/html/database`. Default image: **`ghcr.io/honestlai/picstome:latest`** (`PICSTOME_IMAGE`).
